...
class ObstacleAvoider:
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.cone_front < self.min_dist:
                # chose direction 
                #pub.publish(stop)
                start = now()
                if self.cone_right > self.cone_left:
                    # go right
                    rospy.loginfo("Taking a turn right")
                    while (now() - start) < 2. or self.cone_right < self.min_dist:
                        pub.publish(turn_right)
                        rate.sleep()
                else:
                    # go left
                    rospy.loginfo("Taking a turn left")
                    while (now() - start) < 2. or self.cone_left < self.min_dist:
